...
# Creating an agglomerative (hierachical) clustering
from sklearn.cluster import AgglomerativeClustering
ac = AgglomerativeClustering(n_clusters=3)
ac.fit(iris.data)

plt.figure()   # Creates a new window
plt.title('IRIS - AC Cluster Labels')
plt.xlabel('Speal Length')
plt.ylabel('Sepal Width')
plt.scatter(slength, swidth, c= ac.labels_)

# Plot the deondogram
import scipy.cluster.hierarchy as shc
plt.figure()
plt.title("IRIS Dendogram")
dend = shc.dendrogram(shc.linkage(iris.data, method='ward'))
